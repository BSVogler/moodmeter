...
class Measurement: Codable {
    ...
    /// Necessary to know if the date of this measurement is from yesterday, as it is then shown differently on the VC
	...
	// MARK: Initializers
    /// Initializes object with today's date
	init() {
        date = Date()
        // TODO: Use DateComponents or what is this constructor needed for?!
        if let dateIndex = DataHandler.userProfile.dataset.firstIndex(where: { $0.day == date }) {
            let moodOnDate = DataHandler.userProfile.dataset[dateIndex].mood
            mood = moodOnDate
		}
	}
